...
function Navbar(props) {
  ...
  React.useEffect(() => {
    const apiCall = async () => {
      ...
      let userNotifications = await getUserNotifications(userUsername.data.data.userUsername)
      userNotifications = userNotifications.data.data.getUserNotifications.sort((a, b) => {
  ...
  async function handleClearNotification(time, link){
    ...
    setNotifications(prev => prev.filter(x => x.createdAt !== time && x.link !== link))
  }
        ...
        <ul>
          ...
          <li>
              ...
              <div className="notif-icon-grp">
                <img className="notif" src={Notif} alt="" onClick={() => setShowNotification(prev => !prev)}/>
                {notifications.length > 0 ? <div className="notification-counter">{notifications.length}</div> : null}
              </div>
              {showNotification ?
                <div className="notifications"> 
                <h1 className="notification-header">Notifications</h1>
                <p className="notification-clear-btn" onClick={handleClearNotifications}>
