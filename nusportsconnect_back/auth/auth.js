...
function createAccessToken(user) {
    return(sign({userId : user.id, tokenVersion : user.tokenVersion}, process.env.JWTSECRET, 
        {expiresIn : '15m'}));
}

//Function to create refresh token
function createRefreshToken(user) {
    return (sign({userId : user.id, tokenVersion : user.tokenVersion}, process.env.JWTREFRESHSECRET, 
        {expiresIn : '7d'}));
}
